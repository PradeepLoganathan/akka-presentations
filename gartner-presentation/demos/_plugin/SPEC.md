...
```
Repo:      https://github.com/pradeeploganathan/akka-presentations
Cache:     ~/.akka/akka-presentations/
```

On every run the plugin does:
1. `git clone` the repo if not already cached, or `git pull --ff-only` to update
2. Run `python3 CACHE_DIR/builder/build.py --mode overview` to assemble the current base deck into `~/.akka/akka-presentations/sales-presentation/generated/_plugin/base.html`
...
```
~/.akka/akka-presentations/sales-presentation/generated/_plugin/base.html   — assembled sales deck (markers intact)
~/.akka/akka-presentations/sales-presentation/slides/12-demo/slide.css      — demo section CSS
~/.akka/akka-presentations/sales-presentation/slides/12-demo/slide.html     — demo HTML with {{PLACEHOLDER}} markers
