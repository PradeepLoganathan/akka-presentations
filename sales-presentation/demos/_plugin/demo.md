...
The presentation templates live in the public akka-presentations repository. Clone or update it before every generation so users always get the latest slides, design, and copy.

```
REPO_URL  = https://github.com/pradeeploganathan/akka-presentations
CACHE_DIR = ~/.akka/akka-presentations
```
...
1. If `CACHE_DIR` does not exist:
   ```bash
   git clone https://github.com/pradeeploganathan/akka-presentations ~/.akka/akka-presentations
   ```
2. If `CACHE_DIR` exists:
   ```bash
   git -C ~/.akka/akka-presentations pull --ff-only
   ```
...
```bash
BASE_HTML=~/.akka/akka-presentations/sales-presentation/generated/_plugin/base.html
python3 ~/.akka/akka-presentations/sales-presentation/builder/build.py \
  --mode overview \
  --out "$BASE_HTML"
```
...
```
~/.akka/akka-presentations/sales-presentation/generated/_plugin/base.html   — assembled sales deck with insertion markers
~/.akka/akka-presentations/sales-presentation/slides/12-demo/slide.css      — all #demo-section scoped CSS
~/.akka/akka-presentations/sales-presentation/slides/12-demo/slide.html     — demo section HTML with {{PLACEHOLDER}} markers
~/.akka/akka-presentations/sales-presentation/slides/12-demo/slide.js       — tab switching, SVG diagrams, keyboard nav
```
...
`build.py` already staged images, logos, and resilience alongside `base.html` in `~/.akka/akka-presentations/sales-presentation/generated/_plugin/`. Copy them into `OUT_DIR` so all relative paths in the presentation resolve correctly:

```bash
STAGED=~/.akka/akka-presentations/sales-presentation/generated/_plugin
